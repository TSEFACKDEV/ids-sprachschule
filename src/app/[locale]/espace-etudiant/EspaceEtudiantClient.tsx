...
export default function EspaceEtudiantClient({
  etudiant,
}: {
      ...
      <div className="bg-ids-black text-white">
        <div className="container-ids py-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-ids-gold font-bold text-xs uppercase tracking-widest mb-0.5">
                {t("title")}
              </p>
              <h1 className="font-display text-xl font-bold">
                {t("welcomeMsg")} {etudiant.prenom} {etudiant.nom}
              </h1>
              <p className="text-gray-400 text-xs mt-0.5">
                {etudiant.numeroInscription} — {dateInscription}
              </p>
            </div>
          </div>
          <button
            onClick={handleLogout}
            className="flex items-center gap-2 px-4 py-2 border border-white/20 rounded-xl text-sm text-gray-300 hover:bg-white/10 transition-colors"
          >
      ...
      <div className="container-ids py-8 space-y-6">
        ...
        >
          ...
          <div className="text-right">
            ...
            <span className="inline-block bg-ids-black text-ids-gold font-bold text-lg px-4 py-1.5 rounded-lg">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            <div className="flex items-center gap-3 mb-5">
              ...
              <h2 className="font-display font-bold text-ids-black">
                {t("myGroup")}
              </h2>
            </div>
            {groupe ? (
              <div className="bg-ids-gray rounded-xl p-4">
                <p className="font-bold text-ids-black text-sm mb-3">
                  {groupe.nom}
                </p>
                {[
                  ...
                  >
                    ...
                    <span className="font-semibold text-ids-black text-xs">
                      {value}
                    </span>
                  </div>
                ))}
            ...
            {etudiant.factures.length > 0 ? (
              <div className="space-y-3">
                ...
                {etudiant.factures.map((facture) => (
                  <div
                    key={facture.id}
                    className="flex items-center justify-between p-3 bg-ids-gray rounded-xl"
                  >
                    <div>
                      <p className="font-bold text-ids-black text-xs">
                        {facture.numeroRecu}
                      </p>
                      <p className="text-gray-400 text-xs">
                        {facture.montantVerse.toLocaleString()} FCFA —{" "}
                        {MODE_LABELS[facture.modePaiement] ??
                          facture.modePaiement}
                      </p>
                    </div>
                    <button
                      onClick={() =>
                        handleDownloadReceipt(facture.id, facture.numeroRecu)
                      }
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              [t("levelLabel"), etudiant.niveauAllemand],
            ].map(([label, value]) => (
